...
void red_black_tree_ekle(struct red_black_tree *T,int key){
    ...
    if(T->root==NULL){
      ...
      //  return;        
        ...
        if(d==d->parent->left){
            d->parent->left=x;
            //printf("d->parent->left");
        }
        else{
            d->parent->right=x;
            //printf("d->parent->left degil \n");
...
//Agacin binary searc tree (ikili arama agaci ozelligini saglayip saglamadigini test ediyor.
...
// Agacin red-black-tree ozelligini saglayip saglamadigini test ediyor.
int red_black_tree_mi(struct red_black_tree *T){
 ...
 //   printf("%d",binary_search_tree_mi(T->root,T));
 //   printf("%3d",siyah_dugum_sayilari_kontrolu(T->root));
   
    
 //   printf("%d",yapraktan_koke_kadar_siyah_dugum_say(T->root->left->right->right));
